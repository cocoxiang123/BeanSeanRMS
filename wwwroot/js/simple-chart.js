...
$(document).ready(function () {
    var labels = [];
    var values = [];
    $.get("/Admin/Home/ReservationsData", function (data, status) {
        console.log(data);
        $.each(data, function (key, value) {
            labels.push(value.sittingType);
            values.push(value.totalReservation);
        });
        var total = values.reduce((arr, cur) => arr + cur);
        ...
        $('.chart1').simpleChart({
            title: {
                text: 'Total Reservations In ' + monthNames[(new Date($.now()).getMonth())] + " " + (new Date($.now()).getFullYear()),
                align: 'center'
            },
            type: 'bar',
            layout: {
        ...
        var food = ["Original Double N Cheese Steakburger", 'Cheese Biscuits', 'Spicy Grilled Shrimp Tacos', 'Crispy Caramel Chicken', 'Loaded Potato Skins']
        var orders = [254, 220, 187, 150, 120]

        $('.chart2').simpleChart({
            title: {
                text: 'Top 5 Ordering Menu In ' + new Date($.now()).getFullYear(),
                align: 'center'
            },
            type: 'bar',
            layout: {
